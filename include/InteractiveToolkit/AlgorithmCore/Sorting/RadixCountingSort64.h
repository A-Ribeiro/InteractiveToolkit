        ...
        struct RadixCountingSort<uint64_t>
        {
            ...
            static ITK_INLINE void sort(uint64_t *_arr, uint32_t arrSize, uint64_t *tmp_array = nullptr)
            {
...
#elif defined(ITK_SSE2)

                if (arrSize == 0)
                    return;

                // Counting Sort
                __m128i counting[256][2];
                uint64_t *aux;
...

                {
                    ...
                    // Cleaning counters
                    memset(&counting[0][0], 0, sizeof(__m128i) * 256 * 2);
                    ...
                    for (uint32_t j = 0; j < arrSize; j++)
                    {
                        ...
                        _mm_u32_(counting[index_0][0], 0)++;
                        _mm_u32_(counting[index_1][0], 1)++;
                        _mm_u32_(counting[index_2][0], 2)++;
                        _mm_u32_(counting[index_3][0], 3)++;
                        _mm_u32_(counting[index_4][1], 0)++;
                        _mm_u32_(counting[index_5][1], 1)++;
                        _mm_u32_(counting[index_6][1], 2)++;
                        _mm_u32_(counting[index_7][1], 3)++;
                    }
                    ...
                    for (uint32_t j = 0; j < 256; j++)
                    {
                        __m128i tmp0 = counting[j][0];
                        __m128i tmp1 = counting[j][1];

                        counting[j][0] = acc[0];
                        counting[j][1] = acc[1];

                        acc[0] = _mm_add_epi32(acc[0], tmp0);
                        acc[1] = _mm_add_epi32(acc[1], tmp1);
                    }

                    const __m128i increment_c[] = {
                    ...
                    for (int idx = 0, shift = 0; idx < 8; idx += 1, shift += 8)
                    {
                        ...
                        for (uint32_t j = 0; j < arrSize; j++)
                        {
                            ...
                            int idx_array_pos = idx >> 2;
                            int idx_simd_pos = idx & 0x03;

                            uint32_t out_index = _mm_u32_(counting[bucket_index][idx_array_pos], idx_simd_pos);
                            counting[bucket_index][idx_array_pos] = _mm_add_epi32(counting[bucket_index][idx_array_pos], increment_c[idx_simd_pos]);
