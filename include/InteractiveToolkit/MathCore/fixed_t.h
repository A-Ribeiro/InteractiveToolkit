...
namespace MathCore
{
	...
	namespace FixedPoint
	{
		...
		public:
			...
			inline constexpr fixed_t(const valueT &integerPart, const valueT &fractionalPart) : value(valueT(u_valueT(integerPart) << FRACTIONAL_BITS) + fractionalPart)
			{
				// value = (integerPart << FRACTIONAL_BITS) + fractionalPart;
			}

			template <typename T = StoreSpec, typename std::enable_if<!T::_is_signed::value, bool>::type = true>
			inline constexpr fixed_t(const valueT &integerPart, const valueT &fractionalPart) : value((integerPart << FRACTIONAL_BITS) | (fractionalPart & FRACTIONAL_MASK))
			{
				// value = (integerPart << FRACTIONAL_BITS) | (fractionalPart & FRACTIONAL_MASK);
			}

			inline constexpr fixed_t() : value(0)
			{
				// value = 0;
			}
			...
			template <typename _valueT,
					  typename std::enable_if<
						  ...
						  bool>::type = true>
			inline constexpr fixed_t(const _valueT &integerPart) : value((u_valueT(valueT(integerPart)) << FRACTIONAL_BITS))
			{
				// value = ((valueT)integerPart << FRACTIONAL_BITS);
			}

			inline constexpr fixed_t(const fixed_t &rawValue) : value(rawValue.value)
			{
			...
			inline constexpr fixed_t(const float &v) : value(fixed_t::fromFloat(v).value)
			{
				// *this = fixed_t::fromFloat(v);
			}

			inline constexpr fixed_t(const double &v) : value(fixed_t::fromDouble(v).value)
			{
				// *this = fixed_t::fromDouble(v);
			}

		private:
			inline constexpr fixed_t(const valueT &rawValue, bool is_raw_unused_1, bool is_raw_unused_2) : value(rawValue)
			{
				// value = 0;
			}
		...
		public:
			...
			inline constexpr fixed_t operator-() const
			{
			...
			template <typename T = StoreSpec, typename std::enable_if<!std::is_same<typename T::valueT, int64_t>::value, bool>::type = true>
			inline constexpr fixed_t operator/(const fixed_t &other) noexcept
			{
				// mul_div_T result = mul_div_T(value);
				// result <<= FRACTIONAL_BITS;
				// result /= mul_div_T(other.value);
			...
			inline constexpr fixed_t operator<<(int shift)
			{
			...
			// Comparison block
			//
			inline constexpr bool operator>(const fixed_t &other) const { return value > other.value; }
			inline constexpr bool operator>=(const fixed_t &other) const { return value >= other.value; }
			inline constexpr bool operator<(const fixed_t &other) const { return value < other.value; }
			inline constexpr bool operator<=(const fixed_t &other) const { return value <= other.value; }
			inline constexpr bool operator==(const fixed_t &other) const { return value == other.value; }
			...
			template <typename T = StoreSpec, typename std::enable_if<!T::_is_signed::value, bool>::type = true>
			constexpr valueT integer_part() const
			{
				return value >> FRACTIONAL_BITS;
			}
			template <typename T = StoreSpec, typename std::enable_if<!T::_is_signed::value, bool>::type = true>
			...
			template <typename T = StoreSpec, typename std::enable_if<!T::_is_signed::value, bool>::type = true>
			constexpr valueT fract_part() const
			{
				return value & FRACTIONAL_MASK;
			}

			template <typename T = StoreSpec, typename std::enable_if<T::_is_signed::value, bool>::type = true>
			constexpr valueT sign() const
			{
			...
			template <typename _valueT,
					  typename std::enable_if<
						  std::is_integral<_valueT>::value,
						  bool>::type = true>
			inline constexpr operator _valueT() const
			{
			...
			template <typename _store_type, int _frac_bits,
					  ...
					  typename std::enable_if<
						  ...
						  bool>::type = true>
			inline void operator=(const fixed_t<_store_type, _frac_bits> &vec)
			{
			...
			template <typename _store_type, int _frac_bits,
					  ...
					  typename std::enable_if<
						  ...
						  bool>::type = true>
			inline fixed_t(const fixed_t<_store_type, _frac_bits> &vec)
			{
				*this = vec;
			}
			...
			template <typename _store_type, int _frac_bits,
					  ...
					  typename std::enable_if<
						  !std::is_same<_store_type_b, _store_type>::value ||
							  (_frac_bits != _frac_bits_b),
						  bool>::type = true>
			inline operator fixed_t<_store_type, _frac_bits>() const
			{
